...
fn run() -> Result<(), Flare> {
    ...
    match scname {
        "cat" => {
            ...
            if  subm.is_present("meta") ||
                subm.is_present("req_headers") || subm.is_present("req_body") ||
                subm.is_present("res_headers") || subm.is_present("res_body")
            {
                parts = Parts::falsie();
                if subm.is_present("meta")        { parts.meta        = true; }
                if subm.is_present("req_headers") { parts.req_headers = true; }
                if subm.is_present("req_body")    { parts.req_body    = true; }
                if subm.is_present("res_headers") { parts.res_headers = true; }
                if subm.is_present("res_body")    { parts.res_body    = true; }
            }
            let files = subm.values_of("file").unwrap();
            let mut start = StartPos::default();
            let mut count = usize::max_value();
            if let Some(v) = subm.value_of("index") {
                if files.len() > 1 {
            ...
            if let Some(v) = subm.value_of("offset") {
                if files.len() > 1 {
            ...
            if let Some(v) = subm.value_of("count") {
                if files.len() > 1 {
